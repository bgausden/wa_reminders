...
async function getScheduleItems(startDateTime: Date , endDateTime: Date, offset:number = 0) {
    ...
    if (!response) {
        throw new Error(`Call to ${SCHEDULE_ITEMS_ENDPOINT} failed with no response`)
    }
    if (response.status !== 200) {
        throw new Error(`Call to ${SCHEDULE_ITEMS_ENDPOINT} failed with response ${render(response)}`)
    }
